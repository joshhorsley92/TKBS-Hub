...
// events, EXCLUDING any gap longer than 15 minutes that sits between Claude's
// output and the human's next input — that's the human walking away.
//
// A long agentic run (input → output, 40 minutes of Claude working) counts IN
// FULL. That is deliberate, and it's why token cost is tracked alongside: the
// hour is real, and so is what Claude burned during it.
//
// ── The cost ───────────────────────────────────────────────────────────────
...
export function parseTranscript(path) {
  ...
  for (const line of raw.split('\n')) {
    ...
    // Tool results also arrive as role:'user' — those aren't things a human said.
    if (kind === 'user') {
  ...
  for (let i = 0; i < events.length - 1; i++) {
    ...
    // The idle rule applies ONLY to output → input. A long input → output gap
    // is Claude working, and that is work.
    if (events[i].kind === 'assistant' && gap > IDLE_GAP_SECONDS) idle += gap;
    else worked += gap;
  }
